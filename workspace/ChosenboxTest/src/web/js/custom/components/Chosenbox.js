...
custom.components.Chosenbox = zk.$extends(zul.Widget, {
	$init: function () {
	...
	bind_: function () {
		this.$supers(custom.components.Chosenbox, 'bind_', arguments);
		var n = this.$n(),
			inp = this.$n('inp'),
			chgSel;
	...
	unbind_: function () {
		...
		clearAllData(this);
		this.$supers(custom.components.Chosenbox, 'unbind_', arguments);
	},
	onSize: function () {
		this._fixInputWidth();
	},
...
//				console.log("enter empty-creatable");
...
//				console.log("click empty-creatable");
...
//		console.log("_updateInput");
	...
	_fixSelDisplay: function (hliteFirst, str, fromServer) {
		...
		for (index = 0, element = options[index];
			...
			if (fromServer || !selected) {
				//hoangnv28 - Search element that match str
				var elementContent = element.innerHTML;//
				var indexOfStrVie = elementContent.toLowerCase().indexOf(str.toLowerCase());//
				var indexOfStrEng = this._convertVietnamese(elementContent.toLowerCase()).indexOf(str.toLowerCase());//
				console.log(indexOfStrEng);
				if (!selected &&
						(showAll || str && indexOfStrVie > -1)) {
					if (!found) {
						found = true;
						if (hliteFirst)
					...
					element.style.display = 'none'; // hide if has input and not match
				}
			}
	...
	_convertVietnamese: function(str){
	    str= str.toLowerCase();
	...
	_fixEmptyDisplay: function (type, str, found, exist) {
		...
		if (type && (type.showBlank 
			...
			if (!found) {
				if (this._creatable && !exist && str) {// show create message if input new item and creatable
