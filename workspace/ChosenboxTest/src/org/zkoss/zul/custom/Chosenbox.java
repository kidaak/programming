...
public class Chosenbox extends HtmlBasedComponent {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -5032860399530978017L;
	...
	private transient EventListener<InputEvent> _eventOnSearchingListener;
	private transient EventListener<Event> _eventOnSearchListener;
	...
	static {
		...
		// addClientEvent(Chosenbox.class, Events.ON_CHANGING,
		// CE_DUPLICATE_IGNORE);
		addClientEvent(Chosenbox.class, "onSearching", CE_DUPLICATE_IGNORE
				| CE_IMPORTANT);
		addClientEvent(Chosenbox.class, "onSearch", CE_DUPLICATE_IGNORE
				| CE_IMPORTANT);
	}
	...
	public LinkedHashSet<Object> getSelectedObjects() {
		...
		if (model != null) {
			for (int i = 0; i < _selIdxs.size(); i++) {
				objects.add((String) model.getElementAt((Integer) _selIdxs
						.get(i)));
			}
		}
		return objects;
	...
	/**
	 * Sets the selected objects. It will clear selection first.
	 * 
	 * @param List
	...
	/**
	 * Sets the index of the selected item (-1 if no one is selected). It will
	...
	public void setModel(ListModel<?> model) {
		if (model != null) {
			...
			if (_model instanceof ListSubModel)
				removeEventListener("onSearching", _eventOnSearchingListener);
			_model = null;
		}
		fixIndexs(true, null);
	...
	private void fixIndexs(boolean modelReplaced, ListDataEvent event) {
		...
		if (modelReplaced) {
			...
			case ListDataEvent.INTERVAL_REMOVED:
				for (ListIterator<Integer> lit = _selIdxs.listIterator(); lit
						.hasNext();) {
					Integer i = lit.next();
					if (i > pos1)
						lit.set(i - amount);
		...
		if (model != null) {
			...
			try {
				...
				if (excludeUnselected) {
					for (int i = 0; i < _selIdxs.size(); i++) {
						String s = renderer.render(this,
								model.getElementAt(_selIdxs.get(i)),
								_selIdxs.get(i));
						if ((prefix == null || s.toLowerCase().startsWith(
								prefix.toLowerCase())))
							optList.add(s);
					}
				} else {
					for (int i = 0; i < model.getSize(); i++) {
						String s = renderer.render(this, model.getElementAt(i),
								i);
						if (prefix == null
	...
	private void initDataListener() {
		...
		if (_eventOnSearchingListener == null)
			_eventOnSearchingListener = new EventListener<InputEvent>() {
				@SuppressWarnings("rawtypes")
				public void onEvent(InputEvent event) throws Exception {
					if (getModel() instanceof ListSubModel) {
						updateListContent(
								event.getValue(),
								((ListSubModel) _model).getSubModel(
										event.getValue(), _model.getSize()));
					}
				}
			};
		if (_model instanceof ListSubModel)
			addEventListener("onSearching", _eventOnSearchingListener);

		/*
		 * hoangnv28 Sự kiện thêm mới item
		 */
		if (_eventOnSearchListener == null) {
			_eventOnSearchListener = new EventListener<Event>() {
				@Override
				public void onEvent(Event arg0) throws Exception {
					changeModel(arg0.getData().toString());
				}
			};
			addEventListener("onSearch", _eventOnSearchListener);
		}
	}

	@SuppressWarnings({ "unchecked", "rawtypes" })
	private void changeModel(String newItem) {
		if (_creatable) {
			List<String> listSelectedItem = new ArrayList();
			List<String> newModel = new ArrayList();

			Iterator iterator = getSelectedObjects().iterator();
			while (iterator.hasNext()) {
				listSelectedItem.add(iterator.next().toString());
			}
			listSelectedItem.add(newItem);

			for (int i = 0; i < _model.getSize(); i++) {
				newModel.add(_model.getElementAt(i).toString());
			}
			newModel.add(newItem);

			setModel(new ListModelList(newModel));
			setSelectedObjects(listSelectedItem);
		}
	}
	...
	private Integer getIndexFromValue(String value) {
		...
		for (int i = 0; i < _model.getSize(); i++) {
			System.out.println("model: " + _model.getElementAt(i));
	...
	public void service(org.zkoss.zk.au.AuRequest request, boolean everError) {
		...
		if (selItems != null && !selItems.isEmpty()) {
			String escapeHTML = deEscapeHtml(selItems.get(selItems.size() - 1)
					.toString());
			selItems.remove(selItems.size() - 1);
			selItems.add(escapeHTML);
			request.getData().put("", selItems);
		}

		if (cmd.equals(Events.ON_SELECT)) {
			selItems = (List) request.getData().get("");
			// clear at first
			_selIdxs.clear();
			_jsel = -1;
			...
			Events.postEvent(new Event("onSearch", this, ((List) request
	...
	public String deEscapeHtml(String string) {
		if (string == null) {
			return null;
		}
