...
buttons=new Array();
buttons[0]=new	Array("file", "import3", "export2", "gravity");
buttons[1]=new	Array("edit", ["undo","z"], ["redo","y"], ["copy","c"], ["paste","p"], ["extrude","x"], "scale", ["complement","i"]);
buttons[2]=new	Array("select", ["expand","e"], ["invert","f"], ["all","a"], ["none","d"], "shortestPath", "dominate");
buttons[3]=new	Array("panels", "actions", "navigation");
...
/*function backspaceWidthValue(evt){
	if (evt.keyCode==8){
		evt.preventDefault();
			var widthInputText = document.getElementById('widthInput').nextSibling.firstChild.nodeValue.toString();
			document.getElementById('widthInput').nextSibling.firstChild.nodeValue=widthInputText.substring(0,widthInputText.length-1);
	}
}

function backspaceHeightValue(evt){
	if (evt.keyCode==8){
		evt.preventDefault();
			var heightInputText = document.getElementById('heightInput').nextSibling.firstChild.nodeValue.toString();
			document.getElementById('heightInput').nextSibling.firstChild.nodeValue=heightInputText.substring(0,heightInputText.length-1);
	}
}

function enterWidthValue(evt){
	evt.preventDefault();
	...
	if (k==13){//Enter key (return key)
		document.documentElement.removeEventListener("keydown",backspaceWidthValue,false);
		document.documentElement.removeEventListener("keypress",enterWidthValue,false);
	...
	if(k>=48 && k<=57){
		document.getElementById('widthInput').nextSibling.firstChild.nodeValue+=qk;
	}
}

function enterHeightValue(evt){
	evt.preventDefault();
	if (evt.keyCode) var k= evt.keyCode;
	else var k = evt.charCode;
	var qk=String.fromCharCode(k);
	...
	if (k==13){//Enter key (return key)
		document.documentElement.removeEventListener("keydown",backspaceHeightValue,false);
		document.documentElement.removeEventListener("keypress",enterHeightValue,false);
		document.documentElement.addEventListener("keydown",Graph.kdown,false);
		document.documentElement.addEventListener("keyup",Graph.kup,false);
		document.documentElement.removeChild(document.documentElement.lastChild);
	}
	if(k>=48 && k<=57){
		document.getElementById('heightInput').nextSibling.firstChild.nodeValue+=qk;
	}
}

function changeWidthValue(evt){
	if(evt.button===0 || evt.button===undefined){
		evt.preventDefault();
		evt.stopPropagation();
		document.getElementById('widthInput').nextSibling.firstChild.nodeValue="";
		...
		document.documentElement.addEventListener("keypress", enterWidthValue, false);
		document.documentElement.addEventListener("keydown", backspaceWidthValue,false);
		...
		blur.addEventListener("mousedown", function(evt){evt.preventDefault(); evt.stopPropagation(); document.documentElement.removeEventListener("keypress", enterWidthValue, false); document.documentElement.removeEventListener("keydown", backspaceWidthValue,false); document.documentElement.addEventListener("keydown",Graph.kdown,false); document.documentElement.addEventListener("keyup",Graph.kup,false); document.documentElement.removeChild(evt.currentTarget);}, false);
	}
}

function changeHeightValue(evt){
	if(evt.button===0 || evt.button===undefined){
		evt.preventDefault();
		evt.stopPropagation();
		document.getElementById('heightInput').nextSibling.firstChild.nodeValue="";
		document.documentElement.removeEventListener("keydown",Graph.kdown,false);
		document.documentElement.removeEventListener("keyup",Graph.kup,false);
		document.documentElement.addEventListener("keypress", enterHeightValue, false);
		document.documentElement.addEventListener("keydown", backspaceHeightValue,false);
		var blur=document.createElementNS(xmlns, "rect");
		blur.setAttributeNS(null, "x", 0);
		blur.setAttributeNS(null, "y", 0);
		blur.setAttributeNS(null, "width", window.innerWidth);
		...
		blur.setAttributeNS(null, "opacity", 0);
		document.documentElement.appendChild(blur);
		blur.addEventListener("mousedown", function(evt){evt.preventDefault(); evt.stopPropagation(); document.documentElement.removeEventListener("keypress", enterHeightValue, false); document.documentElement.removeEventListener("keydown", backspaceHeightValue,false); document.documentElement.addEventListener("keydown",Graph.kdown,false); document.documentElement.addEventListener("keyup",Graph.kup,false); document.documentElement.removeChild(evt.currentTarget);}, false);
	}
}*/
...
var enlargeCanvas=function(){
	...
	//var DW = (+document.getElementById('widthInput').nextSibling.firstChild.nodeValue);
	//var DH = (+document.getElementById('heightInput').nextSibling.firstChild.nodeValue);
	//if(!DW) DW=docW;
	//if(!DH) DH=docH;
...
function navigation(){
	...
	var close=function(){
		...
		//document.getElementById('widthInput').removeEventListener("mousedown", changeWidthValue, false);
		//document.getElementById('heightInput').removeEventListener("mousedown", changeHeightValue, false);
	...
	//document.getElementById('widthInput').addEventListener("mousedown", changeWidthValue, false);
	//document.getElementById('heightInput').addEventListener("mousedown", changeHeightValue, false);
...
function scale(evt){
	...
	if(isEmptyObject(Graph.selectionDist)){
		for(var i in Graph.SelectedNodes){
			...
			//var relDistX = ((Graph.Nodes[i].x+(Graph.Nodes[i].width*0.5))-BBox.minX)-((BBox.width-(Graph.Nodes[i].width*0.5))*0.5);
			//var relDistY = ((Graph.Nodes[i].y+(Graph.Nodes[i].height*0.5))-BBox.minY)-((BBox.height-(Graph.Nodes[i].height*0.5))*0.5);
			//Graph.Nodes[i].x = BBox.minX+((BBox.width-(Graph.Nodes[i].width*0.5))*0.5)+(relDistX*factor)-(Graph.Nodes[i].width*0.5);
			//Graph.Nodes[i].y = BBox.minY+((BBox.height-(Graph.Nodes[i].width*0.5))*0.5)+(relDistY*factor)-(Graph.Nodes[i].height*0.5);
...
function message(msg){
	...
	if(msg){
		if(msg.match(/\D/g)){
			...
			dlRect.setAttributeNS(null, 'width', 130);
...
function localExport(gravity){
	var str = export1("nowindow");
	var http_req = false;
	if(gravity)var ds="xsv="+str; 
	else var ds="xsv=<?php header('Content-type: attachment/force-download'); header('Content-disposition: attachment;filename=graph.xml'); ?>"+str; 
	request();
	function request(){
		if(window.XMLHttpRequest){
			http_req = new XMLHttpRequest(); 
		...
		function act(){
			if(http_req.readyState == 4 ){
				//var W =top.wopen("http://cs.sru.edu/dailey09/reno/LIVEVERSIONAUG272009/grapher.html?xsv="+da)
				if(gravity) gravityCont(http_req.responseText);
				else localExportPanel(http_req.responseText);
			}
		}	
	}
}
...
function gravityCont(nfile){
	var http_req = false;
	var data = '/var/www/html/dailey09/LocalGraphs/graph'+nfile+'.php';
	request();
	function request(){
		if(window.XMLHttpRequest){
			http_req = new XMLHttpRequest(); 
		...
		http_req.onreadystatechange = act;
		http_req.open('get', "http://granite.sru.edu/~eje8449/cgi/gravity?"+data);
		http_req.send(null);
		function act(){
			if(http_req.readyState == 4 ){
				//var W =top.wopen("http://cs.sru.edu/dailey09/reno/LIVEVERSIONAUG272009/grapher.html?xsv="+da)
				var W=top.wopen(http_req.responseText, "echo");
			}
		}	
	}
}
