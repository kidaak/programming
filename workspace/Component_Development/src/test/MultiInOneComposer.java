...
public class MultiInOneComposer extends GenericForwardComposer {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 8442066494990929296L;
	Chosenbox chosen;
	Label result;
	List<Integer> selectedIndexes = new ArrayList<Integer>();

	Map<String, String[]> selections = new HashMap(); // map for store each
														// level of list
	Map<String, Integer> lvMap = new HashMap(); // map for mapping an item to
												// its level
	...
	public void onSelect$chosen(Event event) {
		...
		LinkedHashSet<String> s = (LinkedHashSet) ((SelectEvent) event)
				.getSelectedObjects();
	...
	private void changeModel(LinkedHashSet selectedObjects) {
		...
		if (last != null) {
			...
			chosen.setSelectedObjects(l); // set current selection as selected
	...
	private void showResult(LinkedHashSet selectedObjects, boolean onSearch,
			String searchData) {
		Iterator<String> it = selectedObjects.iterator();
